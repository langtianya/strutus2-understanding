...
/**默认在classpath查找XML问 "xwork.xml" ，并且使用它作为XWork的配置
...
public class XmlConfigurationProvider implements ConfigurationProvider {
    ...
    /**从配置文件加载action的配置信息.负责读取<action>标签，并将数据保存在ActionConfig
     * @param actionElement
    ...
    /**从一个XML元素代表它的元素创建PackageConfig。解析包相关的配置，比如返回结果类型，拦截器，相关类
