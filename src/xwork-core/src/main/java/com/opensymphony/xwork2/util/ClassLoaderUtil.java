...
public class ClassLoaderUtil {
    ...
    /**
	 * 加载所有资源与一个给定的名字，可能聚集的从搜索结果上看。如果没有找到任何结果，资源名称前面的“/”，
	 * 再次尝试。此方法将尝试加载资源，使用以下方法（顺序）： 从线程。currentthread()。getcontextclassloader()
	 * 从classloaderutil getclassloader()。类。
	 * <ul>
	 *  callingclass。getclassloader() Load
	 * all resources with a given name, potentially aggregating all results from
	 * the searched classloaders. If no results are found, the resource name is
	 * prepended by '/' and tried again.
	 *
