...
/**
 * ActionContext是一个Action执行的上下文。每个上下文基本上是一个需要执行的action的对象的容器，
 * 如会话、参数、区域设置等。
 * 的ActionContext是线程局部即值存储在ActionContext是独一无二的每个线程。更多信息见ThreadLocal类。
 * 这的好处是你不需要担心用户特定的操作上下文，你只需要它： ActionContext上下文= ActionContext getcontext()；
 * 最后，因为本地使用的线程，你不需要担心让你的操作线程安全
 * <p/>
 * The ActionContext is the context in which an {@link Action} is executed. Each
 * context is basically a container of objects an action needs for execution
 * like the session, parameters, locale, etc.
 * <p>
 * <p/>
 * The ActionContext is thread local which means that values stored in the
 * ActionContext are unique per thread. See the {@link ThreadLocal} class for
 * more information. The benefit of this is you don't need to worry about a user
 * specific action context, you just get it:
 * <p/>
 * <ul>
